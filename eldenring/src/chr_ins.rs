use {
    crate::{
        ai::{CoolTime, add_cool_time, get_cool_time_list},
        emevd,
        mem::*,
        offsets::{
            self,
...
impl ChrIns {
    pub fn get_current_hp(&mut self) -> SysResult<i32> {
        self.get_ptr(ResolvedChrPtr::DataModule)
            .add_offset(data_offsets::HEALTH)
            .read::<i32>()
    }

    pub fn get_max_hp(&mut self) -> SysResult<i32> {
        self.get_ptr(ResolvedChrPtr::DataModule)
    ...
    pub fn is_no_death(&mut self) -> SysResult<bool> {
        self.get_ptr(ResolvedChrPtr::DataModule)
            .add_offset(offsets::chr_ins::data_flags())
            .is_bit_set(bit_flags::NO_DEATH)
    }

    pub fn set_no_damage(&mut self, state: bool) -> SysResult {
        self.get_ptr(ResolvedChrPtr::DataModule)
            .add_offset(offsets::chr_ins::data_flags())
            .set_bit(bit_flags::NO_DAMAGE, state)
    }

    pub fn is_no_damage(&mut self) -> SysResult<bool> {
        self.get_ptr(ResolvedChrPtr::DataModule)
            .add_offset(offsets::chr_ins::data_flags())
            .is_bit_set(bit_flags::NO_DAMAGE)
    }

    pub fn get_max_poise(&mut self) -> SysResult<f32> {
        self.get_ptr(ResolvedChrPtr::SuperArmorModule)
            .add_offset(super_armor_offsets::MAX_POISE)
            .read::<f32>()
    }

    pub fn get_current_poise(&mut self) -> SysResult<f32> {
        self.get_ptr(ResolvedChrPtr::SuperArmorModule)
            .add_offset(super_armor_offsets::CURRENT_POISE)
            .read::<f32>()
    }

    pub fn get_poise_timer(&mut self) -> SysResult<f32> {
        self.get_ptr(ResolvedChrPtr::SuperArmorModule)
            .add_offset(super_armor_offsets::POISE_TIMER)
            .read::<f32>()
    }

    pub fn get_current_animation(&mut self) -> SysResult<i32> {
        self.get_ptr(ResolvedChrPtr::TimeActModule)
            .add_offset(time_act_offsets::ANIMATION_ID)
            .read::<i32>()
    }

    pub fn get_last_act(&mut self) -> SysResult<u8> {
        self.get_ptr(ResolvedChrPtr::AiThink)
            .add_offset(ai_think_offsets::last_act())
            .read::<u8>()
    }

    pub fn set_repeat_last_act(&mut self, state: bool) -> SysResult {
    ...
    pub fn is_repeat_act(&mut self) -> SysResult<bool> {
        self.get_ptr(ResolvedChrPtr::AiThink)
            .add_offset(ai_think_offsets::force_act())
            .read::<u8>()
            .map(|val| val != 0x0)
    ...
    pub fn repeat_act(&mut self, act: u8) -> SysResult {
        self.get_ptr(ResolvedChrPtr::AiThink)
            .add_offset(ai_think_offsets::force_act())
            .write::<u8>(act)
    }

    pub fn force_act(&mut self, act: u8) -> SysResult {
        self.repeat_act(act)?;
    ...
    pub fn is_disable_ai(&mut self) -> SysResult<bool> {
        self.get_ptr(ResolvedChrPtr::CtrlFlags)
            .is_bit_set(bit_flags::DISABLE_AI)
    }

    pub fn get_animation_speed(&mut self) -> SysResult<f32> {
        self.get_ptr(ResolvedChrPtr::BehaviourModule)
            .add_offset(behavior_offsets::ANIMATION_SPEED)
            .read::<f32>()
    }

    pub fn set_animation_speed(&mut self, val: f32) -> SysResult {
        self.get_ptr(ResolvedChrPtr::BehaviourModule)
            .add_offset(behavior_offsets::ANIMATION_SPEED)
            .write::<f32>(val)
    }

    pub fn local_coords(&mut self) -> SysResult<[f32; 3]> {
        self.get_ptr(ResolvedChrPtr::PhysicsModule)
            .add_offset(offsets::chr_ins::physics_offsets::COORDS)
            .read::<[f32; 3]>()
    }

    pub fn hurtbox_radius(&mut self) -> SysResult<f32> {
        self.get_ptr(ResolvedChrPtr::PhysicsModule)
            .add_offset(offsets::chr_ins::physics_offsets::HURT_CAPSULE_RADIUS)
            .read::<f32>()
    }

    pub fn get_distance(&mut self, other: &mut ChrIns) -> SysResult<f32> {
    ...
    pub fn block_id(&mut self) -> SysResult<u32> {
    ...
    pub fn has_speffect(&mut self, speffect_id: u32) -> SysResult<bool> {
    ...
    pub fn get_cool_times(&mut self) -> SysResult<Vec<CoolTime>> {
        let ai_think = self.get_ptr(ResolvedChrPtr::AiThink)?;
        get_cool_time_list(ai_think)
    }

    pub fn add_cool_time(&mut self, animation_id: u32) -> anyhow::Result<()> {
        let ai_think = self.get_ptr(ResolvedChrPtr::AiThink)?;
        add_cool_time(ai_think, animation_id)
    }
    ...
    pub fn get_lua_timers(&mut self) -> SysResult<[f32; 16]> {
        self.get_ptr(ResolvedChrPtr::AiThink)
            .add_offset(ai_think_offsets::LUA_TIMERS_ARRAY)
            .read::<[f32; 16]>()
    }

    pub fn set_as_target(mut self, target_guard: &mut MutexGuard<'static, Target>) -> SysResult {
        write::<u64>(CaveAddr::SavedTargetPointer, self.get_ptr(ResolvedChrPtr::ChrIns)?)?;
    ...
    pub fn handle(&mut self) -> SysResult<u64> {
        self.get_ptr(ResolvedChrPtr::ChrIns)
            .read_offset(offsets::chr_ins::HANDLE)
    }

    pub fn entity_id(&mut self) -> SysResult<u32> {
        self.get_ptr(ResolvedChrPtr::ChrIns)
            .add_offset(offsets::chr_ins::entity_id())
            .read::<u32>()
    }

    pub fn npc_think_param_id(&mut self) -> SysResult<i32> {
        self.get_ptr(ResolvedChrPtr::AiThink)
...
impl ChrIns {
    ...
    pub fn from_handle(handle: u64) -> Option<Self> {
        ...
        let pointer = ResolvedPtr::WorldChrMan
            ...
            .unwrap_or_default();
        if pointer != 0 { Some(Self::new(pointer)) } else { None }
    }
    ...
    pub fn from_entity_id(entity_id: u32) -> anyhow::Result<Self> {
        ...
        fun.patch::<DWORD>("entity_id", entity_id);
        fun.patch::<POINTER>("fn_chr_ins", Function::GetChrInsByEntityId);
        fun.patch::<POINTER>("looked_up", CaveAddr::LookedUpEntityId);

        run_custom_function(fun)?;

        let pointer = read::<u64>(CaveAddr::LookedUpEntityId)?;
        Ok(Self::new(pointer))
    }
