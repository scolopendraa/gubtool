...
fn run_win64_thread(code_address: u64, cave_address: u64, create_thread_pointer: u64) -> Result<()> {
    let pid = pid() ;
    ...
    loop {
        ...
        if start < rip && rip < start + 0x5E03000 {
            ...
            regs.rax = read_unsafe::<u64>(create_thread_pointer)?;
            let asm: [u8; 29] = [
...
pub fn run_win64_thread_wait(code_address: u64, cave_address: u64, create_thread_pointer: u64) -> Result<()> {
    let running_flag = code_address.saturating_sub(1);
    write_unsafe::<u8>(running_flag, 0x1)?;
    run_win64_thread(code_address, cave_address, create_thread_pointer)?;
    let start = Instant::now();
